...
from app.services.llm_client import LLMAPIError, LLMError, LLMRateLimitError, OpenRouterClient, create_llm_client
...
async def websocket_endpoint(websocket: WebSocket, session_id: str | None = None):
    ...
    try:
        ...
        logger.error(f"Database error (session: {actual_session_id}): {e}", exc_info=True)

    except Exception as e:
        logger.error(f"WebSocket error (session: {actual_session_id}): {e}", exc_info=True)
